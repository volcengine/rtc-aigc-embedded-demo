...
int webclient_close(struct webclient_session *session)
{
  ...
  if (session->header && session->header->buffer) {
    hal_free(session->header->buffer);
  }

  if (session->header) {
    hal_free(session->header);
  }

#ifdef CONFIG_WEBCLIENT_HTTPS_SUPPORTED
  if (session->cert_buf) {
    hal_free(session->cert_buf);
    session->cert_buf = NULL;
  }
#endif
